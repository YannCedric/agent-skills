...
5. For system/component diagrams, prefer the compiler prototype:
   ...
   - inspect diagram.review.json and revise the definition or layout hints until the review is publishable for the intended audience
...
- Keep connector sizing compact: ordinary and exception connectors should usually be 2.5-3px strokes, with warning/exception paths carrying color or dash semantics instead of chunky 4px+ weight. Arrow markers should be small, roughly 8x8 or 9x9, never oversized 12x12+ tips for ordinary diagrams.
- Connector labels should not make connections look broken. Prefer no label for obvious short connectors; otherwise place a small off-path label above/beside the connector. Use true inline text-gap groups only when the gap is clearly intentional: a `connector-label` group containing a measured `connector-label-mask` plus centered text, with the connector resuming close to both sides. Do not place raw `flow-label`/`arrow-label` text directly on a path or use ad hoc `label-mask` rectangles.
- Simplify routes before styling them. Avoid long crossing, near-crossing, looping, or bottom-return connectors; split a flow into separate frames or linearize state changes when a clean route needs too many bends.
...
- Treat note groups as first-class compact annotation blocks on the open canvas. Prefer a short title plus 2-4 tight bullets; do not use paragraph-like footer prose or put annotations in cards unless the box is communicating ownership, trust, or another real boundary.
