...
- Note density: keep annotations compact. Use a short heading plus 2-4 scannable lines, not paragraph-like footer prose. If a note becomes a sentence block, shorten it or split the diagram.
...
- Arrows:
  ...
  - default stroke should usually be 2.5-3px; do not make warning or exception routes chunky just to make them noticeable
  - compact arrow markers should be about 8x8 or 9x9; 12x12 tips are too large for ordinary publishable diagrams
...
- Labels: favor fewer, larger labels. Label arrows only when protocol, payload, or state transition matters. For short connectors, prefer no label or a small off-path label above/beside the line so the shaft stays visually continuous. Inline text gaps are acceptable only when the gap reads as deliberate: use a `connector-label` group with a small `connector-label-mask` behind centered text, with the connector resuming close to both sides. Do not drop raw `flow-label` or `arrow-label` text on top of a path. Remove redundant connector labels when branches get tight.
- Routes: avoid forcing long return paths, loops, or connectors that cross or nearly cross other flows. Split lifecycle/status detail into separate linear frames when the cleanest route would require many bends.
...
- connector labels are true `connector-label` groups with masks, not raw text sitting on paths
- no labels overflow their containing boxes or sit tight against box edges
- text is readable at the intended sharing size
- arrows have visible shafts, compact heads, and consistent 2.5-3px connector weight
- the primary reading path is obvious
- connectors do not cross, nearly cross, loop back, or use long bottom/return routes
...
- assumptions/not-implied notes cover important uncertainty as compact annotation blocks, not footer paragraphs
